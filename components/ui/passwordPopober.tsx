...
function PasswordRequirement({ meets, label }: PasswordRequirementProps) {
  return (
    <div
      className={`flex items-center mt-7 text-sm ${
        meets ? 'text-green-600' : 'text-red-300'
      }`}
    >
      {meets ? <Check size={14} /> : <CloseCircle size={14} />}{' '}
      <div className="ml-2.5">{label}</div>
    </div>
  );
}
  ...
  return (
    <div className="relative">
      ...
      {popoverVisible && (
        <div
          ref={popoverRef}
          className="absolute bg-white p-5 rounded shadow-md mt-2 z-50 top-15"
        >
          <div>
