				...
				{problemData &&
					problemData?.map((problem) => (
						<li key={problem.id} className="transition-all duration-500">
							...
							>
								...
								{openProblem !== problem.id && (
									<div className="flex items-center gap-x-3">
										<button onClick={() => setOpenEditProblem(true)} className="flex items-center gap-x-2 text-primary-light">
											<MessageEdit />
											<span>Edit</span>
										</button>
										<button onClick={() => setOpenDeleteProblem(true)} className="flex items-center gap-x-2 text-[#FF3333]">
							...
							{openDeleteProblem && <DeleteProblem isOpen={openDeleteProblem} onClose={() => setOpenDeleteProblem(false)} />}
						</li>
					))}
			</ul>
		</div>
