...
const NotificationPage: React.FC = () => {
	...
	const filteredNotifications =
		activeFilter === 'all' ? mockData : mockData.filter((notification) => notification.type === activeFilter);

	return (
		<div className="mx-auto">
			<div className="flex items-center mb-4">
				{['all', 'notification', 'messages', 'client', 'transaction'].map((filter) => (
					<Button
						key={filter}
						onClick={() => setActiveFilter(filter)}
						className={`mr-2 p-2 border rounded-md  w-[170px] h-[60px] text-[#9C9C9C] font-normal text-lg ${
							activeFilter === filter ? 'bg-primary-light w-[170px] h-[60px] text-white' : 'bg-white'
						}`}
					>
						{filter}
					</Button>
			...
			{filteredNotifications.length > 0 ? (
				...
				<div className="flex flex-col items-center">
					<Image src="/emptystate.svg" alt="No messages" width={500} height={320} className="mb-4" />
