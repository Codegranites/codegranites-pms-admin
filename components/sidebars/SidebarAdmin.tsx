...
const SidebarAdmin = () => {
	...
	return (
		<section className="w-[96px] lg:w-[270px] hover:w-[270px] hover:p-4 transition-all duration-300 py-4 lg:p-4 flex flex-col gap-y-4 items-center lg:items-start fixed h-screen left-0 top-0 overflow-y-auto border-r border-gray-200 sidebar-scroll overflow-x-hidden group select-none">
			...
			<ul className="flex flex-col gap-y-4 pt-8">
				{SIDEBAR_ADMIN_LINKS.map((link) => (
					...
					>
						...
						<span className="max-lg:hidden group-hover:block w-[185px]">
							{link.label}
						</span>
					</li>
				))}
