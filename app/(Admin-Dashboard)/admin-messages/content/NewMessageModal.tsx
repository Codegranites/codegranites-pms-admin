...
const NewMessageModal = () => {
	...
	const isDisabled = !formData.receiver || !formData.subject || !(formData.message.length > 3);
		...
		<>
			<div
				...
				className={cn(
					...
					newMessageModal ? 'opacity-100 duration-500' : 'opacity-0 pointer-events-none duration-300'
				)}
			...
			<div
				...
				className={cn(
					...
					isMaximize ? 'sm:h-[660px] sm:w-[617px] md:w-[700px] duration-500' : 'sm:h-[460px] '
				)}
			>
				<div
					className={cn(
					...
					<div className="flex items-center gap-x-2">
						<button
							...
							onClick={() => {
								...
								window?.localStorage.setItem('new-message', JSON.stringify(formData));
						...
						>
							<span className="absolute z-10 bg-red-700 text-white rounded-md text-xs px-2 py-1 -top-7 -right-[0.6rem] before:content-[''] before:absolute before:-bottom-1 before:left-1/2 before:-translate-x-1/2 before:w-2 before:h-2 before:rotate-45 before:bg-red-700 opacity-0 group-hover/close:opacity-100 transition-all duration-700 group-hover/close:duration-200 pointer-events-none ">
								{clearForm && formData.message.length > 3 ? 'clearing' : isDisabled ? 'close' : 'clear'}
							</span>
							<X size={24} />
						</button>
					</div>
				...
				<form
					...
					className={cn(
						...
						isMaximize ? 'sm:gap-y-6 lg:gap-y-8 gap-y-2 sm:pt-8 pt-4' : 'gap-y-2 pt-4'
					)}
				>
					<div className="flex items-center  w-full relative">
						<span className={cn('absolute left-2 sm:left-3 text-header', isMaximize ? 'sm:font-medium' : '')}>To:</span>
						<input
							type="email"
					...
					<div className="flex w-full justify-between items-center ">
						<div className={cn('flex items-center ', isMaximize ? 'gap-x-4' : 'gap-x-2')}>
							<div>
								<label className="flex items-center gap-x-2   cursor-pointer" htmlFor="file">
							...
							<button type="button">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="25" viewBox="0 0 24 25" fill="none">
									...
									<path
										...
										fill="#4E5256"
									/>
									<path d="M12 18.5C14.28 18.5 16.22 16.84 17 14.5H7C7.78 16.84 9.72 18.5 12 18.5Z" fill="#4E5256" />
									<path
						...
						<button
							...
							className={cn(
								...
								isMaximize ? 'px-8 gap-x-5 h-[50px]' : 'min-[450px]:w-[91px] h-[41px]'
							)}
						>
							<Send size={20} /> Send
						</button>
