...
const MobileMessageSection = () => {
	const { activeMessageTab, setActiveMessageTab, searchMsg, setSearchMsg, filterSearchMsgs, showBtn } = useMessageCtx();
	const { newMessageModal, setNewMessageModal } = useStateCtx();

	const messageLen = 50;
	return (
		<section
			className={cn('flex justify-center w-full  lg:hidden px-2 min-[400px]:py-10 pb-10 max-[400px]:relative', {
		...
		>
			...
			>
				{showBtn ? (
					...
					<MessageAdd1 size={24} className={!showBtn ? 'absolute transition-all !duration-0' : ''} />
				)}
