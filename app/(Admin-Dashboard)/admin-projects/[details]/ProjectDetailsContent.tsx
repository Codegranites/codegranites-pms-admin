...
const ProjectDetailsContent = ({ id }: { id?: string }) => {
	...
	return (
		<>
			...
			<section className="w-full flex flex-col lg:flex-row justify-between gap-4 sm:gap-6 lg:gap-8 xl:gap-12  pb-16 items-center lg:items-start min-h-screen pt-4 overflow-x-hidden">
				<div className="flex w-full flex-col max-w-[687px] gap-y-4 lg:gap-y-6">
					...
					<div className="flex flex-col gap-y-4 w-full px-3 py-6 border border-[#e1e1e1] sm:rounded-xl  ">
						<h3 className="text-lg font-semibold text-header border-b border-[#e1e1e1] pb-2">Project Prototype</h3>
						...
						<div className="flex w-full flex-col gap-y-4 items-center">
							<div className="flex justify-start w-full max-w-[559px] items-center gap-x-2">
								<span className="font-medium max-[364px]:text-sm">Prototype Link:</span>
							...
							<div className="flex w-full items-center justify-end gap-x-3">
								<button type="button" className="flex items-center gap-x-1 text-sm max-[359px]:text-xs">
									{' '}
									<Share size={16} />
									<span>Share</span>
								</button>
