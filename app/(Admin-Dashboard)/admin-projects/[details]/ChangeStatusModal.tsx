...
const ChangeStatusModal = () => {
	...
	const [selectedStatus, setSelectedStatus] = useState<StatusProps['label'] | null>(null);
			...
			{changeStatusModal && (
				<div
					...
					className={cn(
						...
						changeStatusModal
							? 'open-modal-scale rounded-xl md:rounded-2xl'
							: 'scale-0 duration-100 pointer-events-none'
					)}
				>
					<div className="flex items-center justify-between w-full border-b border-[#e1e1e1] pb-4 pl-4 px-4 md:pl-8 ">
						<h3 className="text-lg md:text-2xl font-medium text-header">Change Status</h3>
						<button
							type="button"
							tabIndex={0}
							aria-label="Close"
							...
							{STATUSES.map((status) => (
								<p
									...
									className={cn(
										...
										selectedStatus === 'final' && status.label !== 'final' ? 'opacity-40' : '',
										{
								...
								>
									...
									>
										{selectedStatus === status.label && <span className="bg-primary h-full w-full rounded-full" />}
									</button>
