		...
		<>
			...
			{viewMilestoneModal && (
				<div
					...
					className={cn(
						'py-6   flex flex-col max-[350px]:h-[410px] w-[90%] h-[380px] min-[550px]:w-[500px] md:w-[682px] md:h-[400px] items-center bg-white  fixed top-1/2 left-1/2  z-[999]  transition-all opacity-0 select-none ',
						viewMilestoneModal ? 'open-modal-scale rounded-xl md:rounded-2xl -translate-x-1/2  -translate-y-1/2' : ''
					)}
				>
					<div className="flex items-center justify-between w-full border-b border-[#e1e1e1] pb-4 pl-4 px-4 md:pl-8 ">
						<h3 className="sm:text-lg md:text-2xl font-medium text-header">Milestone Details </h3>
						<button
							type="button"
							tabIndex={0}
							aria-label="Close"
					...
					<div className="flex w-full  pt-8  flex-col gap-y-4 ">
						<p className="text-sm md:text-base px-4">
							<span className="font-medium">Milestone Title:</span> <span>{milestone?.title}</span>
						</p>
						<p className="text-sm md:text-base px-4">
						...
						<p className="text-sm md:text-base px-4 ">
							<span className="font-medium">Milestone Description:</span> <span>{milestone?.description}</span>
						</p>
					</div>
