...
const MakePaymentModal = ({ openModal, setOpenModal, project }: MakePaymentModalProps) => {
	const [selectedPayment, setSelectedPayment] = useState<PaymentProps['label'] | null>(null);
	...
	return (
		<>
			...
			<div
				...
				className={cn(
					'py-6   flex flex-col w-[360px] h-[350px] min-[450px]:h-[380px] min-[450px]:w-[400px] min-[550px]:w-[500px] md:w-[682px] md:h-[459px] justify-between items-start bg-white backdrop-blur-lg fixed top-1/2 left-1/2  -translate-y-1/2 z-[999]  transition-all opacity-0 select-none ',
					openModal
			...
			>
				<div className="flex items-center justify-between w-full border-b border-[#e1e1e1] pb-4 pl-4 px-4 md:pl-8 ">
					<h3 className="text-lg md:text-2xl font-medium text-header">Make Payment</h3>
					<button
						type="button"
						tabIndex={0}
						aria-label="Close"
				...
				<div className="flex w-full h-full pt-2 sm:pt-8 items-center flex-col gap-y-5">
					<p className="text-center text-sm md:text-base">Make payment to enable us continue with this project</p>
					<p className="text-center text-sm md:text-base font-semibold">Select payment type</p>
						...
						{PAYMENTS.map((payment) => (
							<p
								...
								className={cn(
									...
									selectedPayment === 'final' && payment.label !== 'final' ? 'opacity-40' : ''
								)}
							>
								<button
									onClick={() => {
										...
										if (payment.label === 'initial') {
											...
											setAmount(project?.total_cost! - project?.initial_payment!);
								...
								>
									{selectedPayment === payment.label && <span className="bg-primary h-full w-full rounded-full" />}
								</button>
				...
				<div className="flex w-full justify-center h-full pt-4 sm:pt-9">
					...
					>
						Pay {!amount && 'amount'} {amount && <span className="ml-1">{formatPrice(amount)}</span>}
					</button>
				</div>
			</div>
		</>
