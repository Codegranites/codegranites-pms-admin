...
const ClientProfileContent = ({ client_id, client_name }: { client_id?: string; client_name?: string }) => {
	...
	return (
		<>
			...
			<Suspense fallback={<LoadingSpinner />}>
				<section className="w-full flex  flex-col   mb-6 items-start  mt-4 md:mt-8 border border-gray-200 rounded-xl lg:rounded-2xl px-2 sm:px-8 xl:px-10 py-10 md:pb-16">
					...
					<div className="flex flex-col gap-y-2  h-full w-full max-w-[945px] pt-6">
						<h3 className="text-lg md:text-2xl font-medium text-header">Bio:</h3>
						{isBio ? (
							<div className="w-full h-full max-h-[244px] overflow-y-auto py-3 sidebar-scroll">
								<p className="text-sm md:text-base text-header [word-spacing:3px] [line-height:1.9]">{client?.bio}</p>
							</div>
						) : (
							<SkeletonBio />
						)}
			...
			<RemoveClientModal openModal={isRemoveClientModal} setOpenModal={setIsRemoveClientModal} client={client} />
		</>
	);
};
